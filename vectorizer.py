...
def vectorize(
    ...
    reverseall=False,  # Reverse direction of all lines and orders at the end
    negativey=False,  # Whether to flip vertically
):
    ...
    # Flip if necessary
    if negativey: img = img.transpose(pil.Transpose.FLIP_TOP_BOTTOM)
    ...
    while eligibles:
        ...
        while True:
            ...
            # No further continuation found - therefore we add this line and get over again
            direction = 0  # Reset start direction after line finished (avoids small squiggles)
            # If using the outline or boundary technique, check if the line can be looped back
            if ('o' in method or 'b' in method) and len(currentline) > 2:
                kx, ky = currentline[0]
                lx, ly = currentline[-1]
                # Check distance under 1.5 (diagonal)
                if (kx - lx)**2 + (ky - ly)**2 < 2.25: currentline.append(currentline[0])
    ...
    if reverseall:
        ...
        [entry.reverse() for entry in masterlines]
        masterlines.reverse()
...
if __name__ == '__main__':
    #vectorize('shogi.png', 'shogi.svg', method='f', invertpixels=True,
    #          diagnostic=False, svgzoom=.1, splittogrid=(500, 500))
    #vectorize('dalija.png', 'dalija.svg', method='f', invertpixels=True, svgzoom=.4)
    #vectorize('shogitest.png', 'shogitest.svg', method='o', invertpixels=True, svgzoom=4)
    # vectorize('composite.png','composite.svg',method='f',invertpixels=True,svgzoom=0.1)
    vectorize('fields2.png', 'fields2.svg', method='f', invertpixels=True, svgzoom=0.1, splittogrid=(1410, 1410))
